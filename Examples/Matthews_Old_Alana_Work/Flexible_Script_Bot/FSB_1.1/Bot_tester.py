...
while CarryOn == True:
    ...
    try:#ensures graceful failure in the event of an exception
        ...
        r = requests.post('http://0.0.0.0:5111/', json={'current_state': {'bot_states' : {},'state' : {'input' : {'text' : Conversation}}}})

        r.json()#if the response is not proper json, it should crash here

        data = json.loads(r.text)[0]

        print("The bot called " + data["bot_name"] + " responded with '" + data["result"] + "'.")
